...
const ASSETS = {
  ...
  Rafael: "https://promptdocente.vercel.app/testimonials/rafael-sousa.jpg",
  
  Márcia: "https://promptdocente.vercel.app/testimonials/marcia-nogueira.jpg",
...
const TESTIMONIALS = [
  {
    ...
    foto: ASSETS.anaLuiza,
    },
  {
    quote:
    "O que mais ajudou foi organizar objetivo, evidência e nível cognitivo no mesmo fluxo. Fica mais fácil revisar antes de usar.",
    name: "Prof. Rafael Sousa",
    role: "Matemática · Ensino Médio",
    foto: ASSETS.Rafael,
  },
  {
    quote:
      "Na parte de atividades, o guia docente e o material do aluno saem mais coerentes. Não parece texto genérico montado de última hora.",
    name: "Profa. Márcia Nogueira",
    role: "Ciências Humanas · Ensino Médio",
    foto: ASSETS.Márcia,
  },
];
...
function renderTestimonial(index) {
  ...
  const avatarEl = document.querySelector(".testimonial__avatar");

  if (quoteEl) quoteEl.textContent = item.quote;
  if (nameEl) nameEl.textContent = item.name;
  if (roleEl) roleEl.textContent = item.role;
  if (avatarEl) avatarEl.src = item.foto; // 🔥 AQUI MUDA A FOTO
